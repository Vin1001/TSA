...
class stock_predictor:
    ...
    def plot_predict(self,s=6):

        self.__plot_predict(np.append(self.l,np.array((self.__forec(3187.95,s,t=-2,read=False)[:,2]))),self.intv)  
        #print("Hello")  
    ...
    def __forec(self, price_today, n_points, t=-1, read=True):
        ...
        prev_days = np.append(np.array(self.df_test.iloc[t-1,:-1]), price_today)
        prev_days = prev_days[1:]
...
def main():
    print("See the full implementation at https://github.com/Vin1001/TSA/blob/main/notebooks/autoreg%20.ipynb")
